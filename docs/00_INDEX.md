...
| 04 | `04_EVALS.md` | Five acceptance evaluations with worked solutions | stable; all five plus the stretch tasks (1s, 3s, 3p, 3g, 3a Maxwell and 3y Yang-Mills quadratic actions), eval 6 (cubic Galileon, first verified Horndeski member, both EOMs by composition), eval 7 (k-essence / general scalar Horndeski by block decomposition), and eval 8 (nonminimal scalar-tensor by composition, metric sector and the F(phi)R block, both EOMs) implemented and kernel-verified |
