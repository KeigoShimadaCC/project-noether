...
| ST-affine | Palatini scalar-tensor F(phi)R(Gamma) | three variations (g, Gamma, phi), dF non-metricity source (SymPy cross-checked against phi-dependent F backgrounds), boundary assumption, LC limit, genuine residue-zero check on connection EOM via Euler-Lagrange target | M3 |
